...
| ⬜ not started / design only | 10 | `authorization-revoked`, `check-equivalence`, `durable-webhook-delivery`, `remove-turtle-serialization`, `webhook-subscription-bootstrap`, `federation`, `events`, `registry-set-permissions`, `isolated-datasets-and-sparql`, `reorganize-authz-agent-logic` |
| ⬜ follow-up backlog (all items open) | 1 | `revoke-delegation-chain-follow-ups` |

**30 plans total.** All remaining work lives in the 12 non-done plans below —
...
| `reorganize-authz-agent-logic.md` | ⬜ design only (captured, not started) | Grant-generation read plane, in two steps: **step 1** (moved from `docs/sparql.md` candidate 4) injects optional registry listing loaders into data-model's `generateSourceDataGrants` (`DataRegistry.registrations`) and `generateDelegatedDataGrants` (`AgentRegistry.socialAgentRegistrations` at `:197`), with the AA passing SPARQL-backed loaders (`localSparqlTransport` + `listDataRegistrations`/`getDataRegistration`); **step 2** (architectural alternative) relocates the whole grant-generation chain into the authorization-agent, data-model keeping the primitives (types, `GrantRegistry.iriForContained`, `DataRegistry.storageIri`, listing-agnostic `generateChildSourceGrantData`). Honest sizing: step 1 ~15 lines; step 2 moves ~250 lines with HTTP remaining on `factory.role` (AllFromRole), `storageIri` and delegated scans either way | `docs/sparql.md` (candidate 4 — moved here as step 1) |
