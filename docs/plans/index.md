...
| `reorganize-authz-agent-logic.md` | ⬜ design only (captured, not started) | Relocate the grant-generation chain (`generateGrantsForAuthorization` + `generateDataGrants` + `generateSourceDataGrants` + `generateDelegatedDataGrants`) from data-model into the authorization-agent, so the per-registry listing uses the AA's own SPARQL plane (`listDataRegistrations` + `getDataRegistration` over `localSparqlTransport`); data-model keeps the primitives (types, `GrantRegistry.iriForContained`, `DataRegistry.storageIri`, listing-agnostic `generateChildSourceGrantData`). Honest sizing: ~250 lines of grant semantics move; HTTP remains on `factory.role` (AllFromRole), delegated scans and `storageIri` either way. Alternative to `docs/sparql.md` candidate 4's minimal loader-param fix | `docs/sparql.md` (candidate 4 — the minimal alternative, recommended) |
