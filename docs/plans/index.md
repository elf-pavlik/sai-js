...
| `authorization-granting.md` | ⬜ not done (design only — deferred) | The structure-based **granting** pair, extracted from `activity-first-services` steps 7–8 (matching the revocation plan): `recordAuthorization` + `shareResource` ride the structure types (`AuthorizationStructure`/`DataAuthorizationStructure`/`ShareDataInstanceStructure`) whose fields lack `dataModelContext` terms (the term-gap — Decision A settles the carrier first); pre-minted real-id embedded `as:object`, org-session workflows (fixes the shareResource org-context debt), per-grantee consumer choice, `AuthorizationRecorded`/`Revoked` re-pins | `payload-contract-alignment` (execution note 9 — the term-gap); `authorization-revocation` (the `Revoked` carrier consumer); `activity-first-services` (the template + keepers) |
