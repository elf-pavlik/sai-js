...
**Updated after Phase 1 (current):** data-model 200 pass / 11 skip / 10 todo; application 7 pass / 13 skip; authorization-agent gate is typecheck/build; root integration tests unaffected (stable POJO imports only).
...
- `access-description-set.test.ts` — `toEqual({ id })`; `AccessDescriptionSet.loadDescriptions(set, factory)` for the split; `AccessDescriptionSet.findInLanguage(accessNeed.dataset, lang)` (dataset, not resource)
...
1. **Create modules** (all behavior as free fns):
   - `src/shape-tree.ts` — `ShapeTreeData` + `getDescription(tree, lang, factory)`, `getPredicateForReferenced(tree, shapeTree)`, `getShapeTreeDescriptions`-style helpers; `references` extraction in `fromDataset`
   - `src/access-need.ts` — `AccessNeedData` + `getDescription(need, lang, factory)`, `reliableDescriptionLanguages(need, factory)` (needs shape tree languages), child loading
   - `src/access-need-group.ts` — `AccessNeedGroupData` + `getDescription(...)`, `reliableDescriptionLanguages(...)` (reduce across needs)
   - `src/data-instance.ts` (readable) — `DataInstanceData` + `label(data, …)`, `isBlob(data, …)`, `buildChildrenInfo(data, factory, lang)`, `discoverDescriptionResource(iri, factory)`
2. **Factory** — `factory.readable.{shapeTree, accessNeed, accessNeedGroup, dataInstance}` return POJOs.
...
4. **Consumers:**
   - `packages/authorization-agent` — `shapeTree.iri` (POJO field), `dataInstance.dataRegistration!.registeredShapeTree/.iri` (POJO fields); `findAgentsWithAccess`/`formatAgentWithAccess`/`getAccessDetails` adaptations
   - `packages/components` — `services/Authorization.ts` `formatAccessNeed`/`formatAccessNeedGroup`: `accessNeed.getDescription(lang)` / `accessNeed.shapeTree.getDescription(lang)` → module fns; `.iri`/`.required`/`.accessMode`/`.inheritsFromNeed`/`.children` → POJO fields (recursion unchanged)
...
6. **Tests:** rewrite `readable/{shape-tree, access-need, access-need-group, data-instance}.test.ts`; `data-instance.test.ts` (src class) passes with adapted internals; `authorization-agent.test.ts` adaptations
7. **Verify:** same as Phase 1
...
6. **RDF content is identical** — all existing test expectations stay valid; only the serialization changes.
