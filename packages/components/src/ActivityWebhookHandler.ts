...
export class ActivityWebhookHandler extends OperationHttpHandler {
  ...
  private async dispatch(
    ...
    if (isActivityClass(activity, 'InvitationCreated')) {
      // step 1 — the send leg: the RPC pre-minted the invitation id (as:object)
      // and wrote the activity; this workflow PUTs the invitation resource at
      // that id with the channel session, generates the capabilityUrl there
      // and completes. Not a webhook/push type — no accountId pass-through.
      const decoded = S.decodeUnknownSync(InvitationCreated)(activity)
      await client.workflow.start(createInvitation, {
        taskQueue: 'create-grants',
        args: [
          channel.webId,
          // the decoded activity's flat fields build the invitation-to-be
          // (the POJO minus capabilityUrl — generated by the activity)
          {
            id: decoded.object,
